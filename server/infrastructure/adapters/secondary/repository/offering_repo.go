...
func (r *OfferingRepositoryImpl) FindByID(ctx context.Context, id uuid.UUID) (*domain.Offering, error) {
	query := `
		SELECT id, user_id, type, name, description, price, image_url, created_at, updated_at
		FROM offerings
		WHERE id = $1
	`
...
func (r *OfferingRepositoryImpl) Save(ctx context.Context, offering *domain.Offering) error {
	query := `
		INSERT INTO offerings (id, user_id, type, name, description, price, image_url, created_at, updated_at)
...
func (r *OfferingRepositoryImpl) Update(ctx context.Context, offering *domain.Offering) error {
	query := `
		UPDATE offerings
		SET user_id = $1, type = $2, name = $3, description = $4, price = $5, image_url = $6, updated_at = $7
		WHERE id = $8
	`
	_, err := r.pool.Exec(ctx, query,
		offering.UserID, offering.Type, offering.Name, offering.Description,
